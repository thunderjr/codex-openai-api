...
# Like fake_codex.sh, but a turn emits its first delta immediately, then sleeps
# before completing. A single child still serializes turns, so overlap between two
# runs can only come from two separate children (the worker pool).
