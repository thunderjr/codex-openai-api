...
# Like fake_codex.sh, but records this child's PID once at startup so a test can
# observe the worker recycling its app-server child: each recycle spawns a fresh
# process that appends a new PID line to "$CODEX_HOME/pids".
printf '%s\n' "$$" >>"${CODEX_HOME}/pids"
